...
initial_100G_voltage = 6  
final_100G_voltage = 9     # mV
initial_200G_voltage = 3  # mv
final_200G_voltage = 6     # mv

step = 0.05             # mV
interval = 5e-2        # 50 msec.
fixtime = 3           # 0.5 sec.
wait = 1.5
roop = int((final_100G_voltage - initial_100G_voltage) / step)
...
try:
    ...
    for _ in beam_list[:8]:
        ctrl.sis.output_sis_voltage(beam=_, voltage=initial_100G_voltage)
    for _ in beam_list[8:]:
        ctrl.sis.output_sis_voltage(beam=_, voltage=initial_200G_voltage)
   ...
   # pub.publish(msg_hot)
    ...
    for vol in range(roop+1):
        for _ in beam_list  :
            ctrl.sis.output_sis_voltage(beam=_, voltage=vol*step+initial_100G_voltage)
        time.sleep(fixtime)
