...
class slider(object):
    ...
    def __init__(self):
        ...
        self.axis = ['x', 'y', 'z']
        for i in axis:
            pub_ptp_onoff.append(rospy.Publisher('/cpz7415v_rsw{0}_{1}_ptp_onoff_cmd'.format(self.rsw_id, i), Bool, queue_size=1))
            pub_length.append(rospy.Publisher('/cpz7415v_rsw{0}_{1}_pulse_num_cmd'.format(self.rms_id, i), Int64, queue_size=1))
            pub_speed.append(rospy.Publisher('/cpz7415v_rsw{0}_{1}_fh_speed_cmd'.format(self.rms_id, i),Int64, queue_size=1))
            pub_home.append(rospy.Publisher('/cpz7415v_rsw{0}_{1}_move_to_home'.format(self.rms_id, i), Bool, queue_size=1))
            
        self.sub = [rospy.Subscriber('/cpz7415v_rsw0_x_onoff', Bool, self.callback_x), rospy.Subscriber('/cpz7415v_rsw0_y_onoff', Bool, self.callback_y), rospy.Subscriber('/cpz7415v_rsw0_z_onoff', Bool, self.callback_z)]
    ...
    def ptp_onoff(self, axis = 0, onoff = 1):
        pub_ptp_onoff[axis].publish(onoff)
        return
        
    def set_length(self, axis = 0, len = 1):
        pub_length[axis].publish(len * 100)
        return

    def set_speed(self, axis = 0, speed = 1000):
        pub_speed[axis].publish(speed)
        return    

    def move_to_home(self, axis = 'x', cmd = True):
        pub_home[axis].publish(cmd)
        return

    def callback(self, req):
        self.move_onoff = req.data # True/False
        return
	...
	def initialize(self, x = 90, y = 30, length = 70, strk = 1 ,speed = 1000, dir = '/home/amigos/beam_pattern/data/2018_10_20_script_test'):
		...
		axis = [0,1]
		for i in axis:
			set_speed(axis = axis, speed = speed) #速度設定
			move_to_home(axis = axis, cmd = True) #原点復帰
	...
	def on_ptp(self, axis = 0, len = 1):
    	set_length(axis = axis, len = len)
		ptp_onoff(axis_num = axis, onoff = 1)
	...
	def measure(self, x = 90, y = 30, length = 70, axis = 'x', strk = 1, direction = 'ccw', tool = 'nothing', sleep_measure = 5, beam_num = 'nothing'):
		...
		set_length(axis = axis_num, len = 1)
	...
	def slide_x_go(self, x = 90, y = 30, length = 70, strk = 1 ,speed = 1000, tool = 'nothing', sleep_measure = 5, dir = '/home/amigos/beam_pattern/data/2018_10_20_script_test', beam_num = 'nothing'):
		initialize(x = x, y = y, length = length, strk = strk ,speed = speed, dir = dir)
		on_ptp(axis = 0, len = x)
		measure(x = x, y = y, length = length, axis = 'x', strk = strk, direction = 'ccw', tool = tool, sleep_measure = sleep_measure, beam_num = beam_num)
		finalize()
		return
	...
	def slide_x_back(self, x = 90, y = 30, length = 70, strk = 1 ,speed = 1000, tool = 'nothing', sleep_measure = 5, dir = '/home/amigos/beam_pattern/data/2018_10_20_script_test', beam_num = 'nothing'):
		initialize(x = x, y = y, length = length, strk = strk ,speed = speed, dir = dir)
		on_ptp(axis = 0, len = self.last_x)
		measure(x = x, y = y, length = length, axis = 'x', strk = - strk, direction = 'cw', tool = tool, sleep_measure = sleep_measure, beam_num = beam_num)
		finalize()
		return
	...
	def slide_y_back(self, x = 90, y = 30, length = 70, strk = 1 ,speed = 1000, tool = 'nothing', sleep_measure = 5, dir = '/home/amigos/beam_pattern/data/2018_10_20_script_test', beam_num = 'nothing'):
		initialize(x = x, y = y, length = length, strk = strk ,speed = speed, dir = dir)
		on_ptp(axis = 1, len = -y)
		measure(x = x, y = y, length = length, axis = 'y', strk = -strk, direction = 'cw', tool = tool, sleep_measure = sleep_measure, beam_num = beam_num)
		finalize()
		return
	...
	def slide_y_go(self, x = 90, y = 30, length = 70, strk = 1 ,speed = 1000, tool = 'nothing',sleep_measure = 5, dir = '/home/amigos/beam_pattern/data/2018_10_20_script_test', beam_num = 'nothing'):
		initialize(x = x, y = y, length = length, strk = strk ,speed = speed, dir = dir)
		on_ptp(axis = 1, len = 190 - self.last_y)
		measure(x = x, y = y, length = length, axis = 'y', strk = strk, direction = 'ccw', tool = tool, sleep_measure = sleep_measure, beam_num = beam_num)
		finalize()
		return	
